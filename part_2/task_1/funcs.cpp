...
void WriteErrorToFile(int N, double err, const std::string& filename) {
...
void ClearFile(const std::string& filename) {
    ...
    if (outFile.is_open()) {
        ...
        std::cerr << "Unable to open file for clearing: " << filename << std::endl;
    }
}
...
double ErNorm(const std::vector<double>& A, const std::vector<double>& b, std::vector<double>& x, int N, std::vector<double>& mem){
    ...
    for (int i = 0; i < N-1; ++i){
        ans += ((b[i] - mem[i]) * (b[i] - mem[i]) * (1./(N-0.5)));
    }
    // ans *= 1./(N-0.5);
...
//     for (int i = 0; i < N; ++i) {
//         double diff = b[i] - mem[i];
//         ans += diff * diff;
//         std::cout << "b[" << i << "] = " << b[i] << ", mem[" << i << "] = " << mem[i] << ", diff = " << diff << std::endl;
//     }
